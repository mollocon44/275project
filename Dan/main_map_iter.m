...
for j = 1:map_iter
    ...
    for i = 1:test_iter
        ...
        time(j,i,1) = min_dist_td;
        time(j,i,2) = max(smd_pd);
        time(j,i,3) = max(smd_std);

        min_dist_pd(j,i) = min(dist_history_pd);
        min_dist_std(j,i) = min(dist_history_std);    

        if i == test_iter
            time(j,i,:)
            ave(j,1) = mean(time(j,:,1));
            ave(j,2) = mean(time(j,:,2));
            ave(j,3) = mean(time(j,:,3));

            std_dev(j,1) = std(time(j,:,1));
            std_dev(j,2) = std(time(j,:,2));
            std_dev(j,3) = std(time(j,:,3));
        ...
        for s = 1:salesmen
                ...
                title(sprintf('Total Distance for cost(percent difference) = %1.4f',min_dist_pd(j,i)));
                hold on;
        end
        ...
        for s = 1:salesmen
                rte = [1 opt_rte_pd.ch{s} 1];
                plot(xy(rte,1),xy(rte,2),'.-','Color',clr(s,:));
                title(sprintf('Total Distance for cost(standard deviation) = %1.4f',min_dist_std(j,i)));
                hold on;
        end
