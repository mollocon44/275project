function [opt_rte, min_dist, smd] = mtsp_ga_multi_ch3(xy,dmat,salesmen,min_tour,max_tour,tw,pop_size,num_iter,use_complex,show_prog,show_res)
...
%         if (pd_sd12 > 20)
%             cost(p) = total_dist(p) + (pd_sd12-20)*20;
%         end
%         if (pd_sd23 > 20)
%             cost(p) = total_dist(p) + (pd_sd23-20)*20;
%         end
%         if (pd_sd13 > 20)
%             cost(p) = total_dist(p) + (pd_sd13-20)*20;
%         end
      cost(p) = d;
   end
